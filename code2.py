...
# ───────────────────────────────────────────────────────────────
# PLOTLY THEME  — shared layout applied to every figure
# ───────────────────────────────────────────────────────────────
...
.kpi:hover {{ border-color: {C["border2"]}; transform: translateY(-2px); }}
.kpi-glow {{
...
.pcard:hover {{ border-color:{C["accent"]}55; transform:translateY(-1px); }}
...
.stDivider {{ border-color:{C["border2"]} !important; opacity:0.6; }}
...
# ───────────────────────────────────────────────────────────────
# EXCEL EXPORT  (openpyxl — styled workbook)
# ───────────────────────────────────────────────────────────────

def build_excel(df: pd.DataFrame) -> bytes:
    """Create a fully styled Excel workbook and return as bytes."""
    wb = Workbook()
    ws = wb.active
    ...
    title_cell = ws["A1"]
    title_cell.value = "ExpiryGuard — Inventory Report"
    title_cell.font  = Font(name="Calibri", bold=True, size=16, color="F5A623")
    title_cell.fill  = PatternFill("solid", fgColor="080D16")
    ...
    dt_cell.font  = Font(name="Calibri", size=10, color="64748B")
    dt_cell.fill  = PatternFill("solid", fgColor="0E1520")
    dt_cell.alignment = Alignment(horizontal="center")
    ws.row_dimensions[2].height = 20

    # ── Header row ──
    header_fill = PatternFill("solid", fgColor="131C2E")
    ...
    for ri, (_, row) in enumerate(display_df.iterrows(), 4):
        ...
        row_fill = PatternFill("solid", fgColor="0E1520") if ri % 2 == 0 else PatternFill("solid", fgColor="131C2E")
        for ci, col in enumerate(cols, 1):
    ...
    for ri, status in enumerate(STATUS_ORDER, 3):
        ...
        ws2.cell(ri,2,cnt).font     = Font(name="Calibri",size=10,color="E2E8F0")
        ws2.cell(ri,2).fill         = fil
        ws2.cell(ri,2).alignment    = Alignment(horizontal="center")
        ws2.cell(ri,2).border       = border
        ws2.cell(ri,3,f"₹{loss:,.2f}").font  = Font(name="Calibri",size=10,color="E2E8F0")
...
with st.sidebar:
    ...
    st.markdown(
        ...
        f'  <div class="eg-name">🛡 ExpiryGuard</div>'
...
if page == "📊  Dashboard":
    ...
    st.markdown(
        ...
        f'    <div class="rb-sub">{exp_cnt if (exp_cnt := expired) else 0} expired · '
        f'{critical} critical · {soon} expiring soon</div>'
        f'  </div>'
    ...
    # ── Row 3: Top-10 expiry timeline + Risk scatter ──
...
# ═══════════════════════════════════════════════════════════════
    ...
    # ── Timeline chart — all at-risk products ──
    ...
    with tab_dl:
        ...
        with d3:
            ...
            st.download_button(
                "⬇  Excel Report (.xlsx)",
                data=xlsx_bytes,
                file_name=f"ExpiryGuard_Report_{today_str}.xlsx",
...
# ═══════════════════════════════════════════════════════════════
    ...
    if filt.empty:
        ...
        # Mini chart for filtered results
        if len(filt) > 1:
            st.markdown('<div class="glass">', unsafe_allow_html=True)
